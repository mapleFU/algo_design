...
void InsertCommand::excute() {
    ...
    lastnode = student_list.insert(position, Student(i_num, std::move(i_name),
...
void DeleteCommand::excute() {
    ...
    std::cin >> number;
    // 可以考虑引用
    lastnode = student_list.pop(number, &delete_pos);
    std::cout << "你所删除的考生的信息是: " << lastnode->student << '\n';
}

void DeleteCommand::undo() {
    student_list.insert(delete_pos, lastnode->student);
}
...
void UpdateCommand::excute() {
    ...
    // TODO: 右值引用
    std::cin >> i_num >> i_name >> i_gender >> i_age >> i_test_type;
    auto newnode = Student(i_num, std::move(i_name),
            std::move(i_gender), i_age, std::move(i_test_type));
    deleted_student = std::make_shared<Student>(curnode->student);
    curnode->student = newnode;
    lastnode = curnode;
}

void UpdateCommand::undo() {
    lastnode->student = *deleted_student;
}
