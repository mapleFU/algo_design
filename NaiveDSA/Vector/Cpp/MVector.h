...
/// 2. 对象赋值比之前的复杂
    ...
    private:
        // tcpppl 建议用一个 base class 专门来分配这个
        //    elem    space      last
        //    |       |          |
    ...
    class VectorBase {
    public:
        VectorBase(int base_sz);
        VectorBase(const VectorBase& vb) = delete;
        VectorBase(VectorBase&& vb);
        VectorBase&operator=(const VectorBase& vb) = delete;
